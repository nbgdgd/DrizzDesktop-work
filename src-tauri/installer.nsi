; Drizz Desktop: the default Tauri 1.6 NSIS template (tauri-cli 1.6.3),
; unchanged except for the marked "Drizz Desktop" block in Section
; Uninstall. Keep it in step with the CLI version when upgrading Tauri.
Unicode true
...
Section Uninstall
  ...
  ; Drizz Desktop: the pet sets per-app left/right gains in the Windows
  ; mixer (balance.rs); put every app back to full before the program that
  ; could do it is gone. Silent, a few hundred milliseconds.
  ExecWait '"$INSTDIR\${MAINBINARYNAME}.exe" --reset-audio'
...
FunctionEnd
Failed to setup handlebar template-setupbundle/makensis.exe to produce -V1-V2-V3-V4NSISDIRNSISCONFDIRerror running makensis.exenshCustom tauri messages for  are not translated.
