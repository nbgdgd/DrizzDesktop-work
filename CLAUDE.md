...
- `src/panel/` — settings window (Radix Themes + Lucide), `pages/Welcome.tsx` first run, `pages/Ears.tsx` ear care. Rust: `main.rs` commands, `native.rs` Win32, `chores.rs` cursor nudge / temp / weather, `env.rs` sound (headphones, per-channel dB), `balance.rs` per-app balance in the mixer, `guard.rs` volume ceiling, `tap.rs` WASAPI loopback (real level for the dose, spike ducking, bands/beat as `audio-tap`) with pure `dsp.rs` (test it with `rustc --edition 2021 --test src-tauri/src/dsp.rs`). `music.ts` beat clock + `eq.ts` equalizer at the feet. `headset.rs` headphone name, Bluetooth battery, pause on unplug, per-headphone profiles (`earsProfiles`); `hearing.ts` + `pages/HearingCheck.tsx` hearing check. `focus.ts` pomodoro (`Director.hushed` = shift or focus: only priority lines) and water/posture/eyes reminders.
