...
- **Язык.** Русский или английский (интерфейс, реплики, трей, подсказки ИИ). Русские реплики как были — с матом. Английские по умолчанию без мата; переключатель «Мат в английских репликах» включает его. Команды в разговоре понимаются на обоих языках.
- **Первый запуск** — экран приветствия в окне настроек: язык, питомец, самое важное (мат для английского, игры с курсором, толчок курсора, звуки, «Береги уши», слежка за консолями, автозапуск, имя). Старая табличка с матом убрана.
- **Фокус и напоминания** (`focus.ts`). Кнопка «Фокус 25 мин» на главной странице панели: питомец молчит (только важное), не трогает курсор, над ним таймер; потом зовёт на перерыв (каждый четвёртый длинный) с кнопками «Пропустить перерыв» и «Хватит», после перерыва зовёт обратно. Напоминания: вода, осанка, глаза 20-20-20 - считают только время за компьютером, пять минут отсутствия отдыхают глаза и спину, во время фокуса ждут перерыва, в тихие часы и в полноэкранных программах молчат. Настройки в «Поведение → Фокус и напоминания».
- **Музыка** (`music.ts`, `eq.ts`). Когда играет музыка или видео, по бокам от питомца прыгают две маленькие колонки-эквалайзера (басы ближе к нему), только когда он стоит, и никогда не свешиваются за край окна или экрана. Танцует и кивает в такт: темп и доля ищутся в самом звуке. Звук не записывается и никуда не уходит. Выключается в «Поведение → Реплики и звук».
- **Береги уши** (`ears.ts`, вкладка «Уши»). В наушниках (форм-фактор устройства Windows: Headphones/Headset; Bluetooth можно пометить вручную) питомец считает недельную дозу звука отдельно для каждого уха по стандарту ВОЗ/МСЭ H.870: 100 % — 80 дБА 40 часов в неделю (бережный режим — 75 дБА), +3 дБ вдвое сокращают время. Уровень меряется по тому, что реально играет (захват звука Windows, A-кривая, каждый канал отдельно; `tap.rs`), плюс громкость Windows и максимальная громкость наушников (по умолчанию 100 дБ); пока захват не запустился - оценка по ползунку. Внезапно громкие места (крик, реклама на 10+ дБ громче того, что играло) приглушаются на пару секунд, потом громкость возвращается, если её не трогали. Реплики: наушники подключены, первая порция звука за день, громко несколько минут и резкий скачок (с кнопкой «Убавить»), перерыв через час и настойчивее через полтора, «уши отдохнули», ночь в наушниках, 50/80/100 % недельной нормы, одно ухо слушает больше другого. Баланс левого/правого канала (Windows сохраняет общий ползунок громкости), «Отдых ушей» — одно ухо тише, по очереди каждые N минут; при выходе баланс возвращается. По желанию питомец сам убавляет громкость, когда норма кончилась. Строка «Уши: N% недельной нормы» — в карточке по правому клику. Наушники по имени (`headset.rs`): заряд Bluetooth-наушников на вкладке «Уши» и реплики на 20 % и 10 %; сняли наушники - плеер на паузу; настройки (потолок, громкость при подключении, максимум, баланс) запоминаются для каждых наушников и ставятся сами. Ночью (с «ночного часа» до 6:00) потолок громкости ниже. Проверка слуха: писки по ушам на пяти частотах, подсказка баланса, это ориентир, а не диагноз (`hearing.ts`). По понедельникам - отчёт за прошлую неделю; достижения «Уши в норме», «Бережёт уши», «Перерыв по расписанию», «Проверил слух».
- **Погода по городу или стране** (`weather.ts`, `places.ts`). В «Поведении» → «Погода»: поиск города (геокодер Open-Meteo, уходит только набранное название) или страна из списка (погода по столице), координаты — по желанию. Питомец говорит, что изменилось с прошлой проверки: пошёл дождь, кончился, гроза, снег, туман, жара, мороз — с названием места и температурой; при первой проверке за день — сводка. Над ним маленькое облачко: капли падают на зонтик и разбрызгиваются, снежинки кружатся, в грозу мелькает молния; зонтик перерисован в мультяшном стиле, облачко реплики поднимается над ним.
- **О программе и благодарности** (`credits.ts`). Страница «О программе»: версия, где хранятся данные, авторы и условия каждого персонажа со ссылками, библиотеки, звуки, шрифт, данные. Под каждым персонажем в выборе — строка об источнике и лицензии.
- **Перед публикацией** — `docs/RELEASE.md`: что блокирует публикацию, какие баги найдены и исправлены, что проверить на чистой Windows. Журнал падений `crash.log` (паники Rust в любом потоке и необработанные ошибки JS) — всегда, без включения диагностики.
- **Фонарик ночью** — маленький фонарь с горящей линзой и пятном света на полу; полупрозрачный луч, который на тёмном столе выглядел серым клином, убран.
...
Модули: `native.rs` — геометрия/WinEvents/input idle/хуки ввода; `env.rs` — громкость, звук, буфер, тема, память, диск, число окон; `usage.rs` — время по программам; `game.ts` — прокачка; `apps.ts` — имена программ; `observe.rs` — системные сигналы/медиа; `director.ts` — реакции; `movement.ts` — движение; `animation.ts` — атлас; `PetScene.ts` — сцена; `dpi.ts` — логический/физический размер холста; `diag.ts` — диагностика; `dialogue.ts` — реплики; `storage.rs` — сохранение, DPAPI, файл диагностики; `src/panel/` — окно настроек (Radix Themes); `cursorplay.ts` — курсор; `antics.ts` — долгие занятия; `chronicle.ts` — память и достижения; `character.ts` — характеры; `calendar.ts` — время суток и даты; `voice.ts` — синтез звуков; `pose.ts`/`effects.ts`/`balloon.ts`/`props.ts` — отрисовка; `chores.rs` — толчок курсора, Temp, погода; `workhud.ts` — панель смены; `quickcard.ts` — карточка по правому клику; `i18n.ts`/`i18n.en.ts`/`lines.en.ts`/`swear.ts` — языки; `ears.ts` — уши; `tap.rs`/`dsp.rs` — захват звука: уровень, всплески, полосы и такт; `music.ts`/`eq.ts` — танец в такт и эквалайзер у ног; `weather.ts`/`places.ts` — погода; `credits.ts` — благодарности; `integration.rs` — локальные события.
